...
define(['jquery', 'websockets/binary_websockets', 'windows/windows', 'cashier/currency', 'common/rivetsExtra', 'lodash', 'moment'], function ($, liveapi, windows, currencyDialog, rv, _, moment) {
    ...
    li.click(function () {
      if (!deposit_win) {
        liveapi.cached.authorize().then(function (data) {
          ...
          return require(['text!cashier/deposit.html'], init_deposit_win);
        }).catch(error_handler);
      } else deposit_win.moveToTop();
    });
  }
  ...
  function init_state(root) {
    ...
    state.standard_methods.iframe_loaded = function () {
      if (state.user.cashier_url) state.standard_methods.iframe_visible = true;
    };
    ...
    state.payment_agents.get_commission_text = function (agent) {
      ...
      return agent.deposit_commission + '% ' + 'deposits'.i18n() + ' / ' + agent.withdrawal_commission + '% ' + 'withdrawals'.i18n();
    };
    ...
    var residence_promise = liveapi.send({ get_settings: 1 }).then(function (data) {
    ...
    residence_promise.then(function () {
      if (!state.user.residence) return { paymentagent_list: { list: [] } };
