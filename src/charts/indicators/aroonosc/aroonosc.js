    ...
    function init( containerIDWithHash, _callback ) {
        ...
        require(['text!charts/indicators/aroonosc/aroonosc.html'], function ( $html ) {
            ...
            $html.find('#aroonosc_level_add').click(function () {
                require(["charts/indicators/aroonosc/aroonosc_level"], function(aroonosc_level) {
                ...
                buttons: [
                    {
                        text: "OK",
                        click: function() {
                            if (!isNumericBetween($html.find(".aroonosc_input_width_for_period").val(),
                                            parseInt($html.find(".aroonosc_input_width_for_period").attr("min")),
                                            parseInt($html.find(".aroonosc_input_width_for_period").attr("max")))) {
                                require(["jquery", "jquery-growl"], function ($) {
                                    $.growl.error({
                                        message: "Only numbers between " + $html.find(".aroonosc_input_width_for_period").attr("min")
                                                + " to " + $html.find(".aroonosc_input_width_for_period").attr("max")
                                                + " is allowed for " + $html.find(".aroonosc_input_width_for_period").closest('tr').find('td:first').text() + "!"
                                    });
                                });
                                return;
                            }
