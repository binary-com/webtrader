    ...
    function init( containerIDWithHash, _callback ) {
        ...
        require(['text!charts/indicators/aroon/aroon.html'], function ( $html ) {
            ...
            $html.find('#aroon_level_add').click(function () {
                require(["charts/indicators/aroon/aroon_level"], function(aroon_level) {
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            if (!isNumericBetween($html.find(".aroon_input_width_for_period").val(),
                                ...
                                require(["jquery", "jquery-growl"], function ($) {
                                    $.growl.error({
                                        message: "Only numbers between " + $html.find(".aroon_input_width_for_period").attr("min")
                                                + " to " + $html.find(".aroon_input_width_for_period").attr("max")
                                                + " is allowed for " + $html.find(".aroon_input_width_for_period").closest('tr').find('td:first').text() + "!"
                                    });
                                });
                                return;
                            }
