    ...
    function init( containerIDWithHash, _callback ) {
        ...
        require(['text!charts/indicators/var/var.html'], function ( $html ) {
            ...
            $html.find('#var_level_add').click(function () {
                require(["charts/indicators/level/level"], function(var_level) {
                ...
                buttons: [
                    {
                        text: "OK",
                        click: function() {
                            if (!_.inRange($html.find(".var_input_width_for_period").val(),
                                            parseInt($html.find(".var_input_width_for_period").attr("min")),
                                            parseInt($html.find(".var_input_width_for_period").attr("max")))) {
                                require(["jquery", "jquery-growl"], function ($) {
                                    $.growl.error({
                                        message: "Only numbers between " + $html.find(".var_input_width_for_period").attr("min")
                                                + " to " + $html.find(".var_input_width_for_period").attr("max")
                                                + " is allowed for " + $html.find(".var_input_width_for_period").closest('tr').find('td:first').text() + "!"
                                    });
                                });
                                return;
                            }
