...
SUM = function (data, options, indicators) {
    ...
    for (var index = 0; index < data.length; index++) {
        if (index >= (this.options.period - 1)) {
            var sum = CalculateSUMValue.call(this, data, index);
...
CalculateSUMValue = function (data, index) {
    /* SUM :
...
SUM.prototype.addPoint = function (data) {
    ...
    var sum = CalculateSUMValue.call(this, this.priceData, this.priceData.length - 1);
...
SUM.prototype.update = function (data) {
    ...
    var sum = CalculateSUMValue.call(this, this.priceData, index);
