            ...
            require(["charts/chartWindow"], function(chartWindow) {
                ...
                if (!validation.validateIfNoOfChartsCrossingThreshold(chartWindow.totalWindows())) {
                    require(["jquery", "jquery-growl"], function($) {
                        $.growl.error({
                            message: "No more charts allowed!"
                        });
                    });
                    return;
                }
                ...
                require(["common/util"], function() {
                    ...
                    if (timeperiodObject) {
                        ...
                        if (validation.validateNumericBetween(timeperiodObject.intValue(), parseInt($("#timePeriod").attr("min")), parseInt($("#timePeriod").attr("max")))) {
                            ...
                            if (delayAmount <= (timeperiodObject.timeInSeconds() / 60)) {
                                ...
                                require(["jquery", "jquery-growl"], function($) {
                                    ...
                                    $.growl.error({
                                        message: "Charts of less than "
                                            //Convert to human readable (in minutes) format
                                            + convertToTimeperiodObject(delayAmount + 'm').humanReadableString() + " are not available for the " + displaySymbol + "."
                                    });
                                });
                            }
                        } else {
                            require(["jquery", "jquery-growl"], function($) {
                                $("#timePeriod").addClass('ui-state-error');
                                $.growl.error({
                                    message: "Only numbers between " + $("#timePeriod").attr("min") + " to " + $("#timePeriod").attr("max") + " is allowed for " + $("#units option:selected").text() + "!"
                                });
                            });
                        }
                        ...
                        require(["jquery", "jquery-growl"], function($) {
                            $("#timePeriod").addClass('ui-state-error');
                            $.growl.error({
                                message: "Only numbers between 1 to 100 is allowed!"
                            });
                        });
                    }
                            ...
                            $("#units").change(function() {
                                if ($(this).val() == 't') {
                                    ...
                                    var max = {
                                        ...
                                        h: 23,
                                        d: 3
                                    }[val] || 120; /* restric range for minute,hour,day*/
